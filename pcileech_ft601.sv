...
module pcileech_ft601(
    ...
    assign din_req_data = !rst && (data_queue_count < 4);
    assign FWD          = !rst && !FT601_TXE_N && (data_queue_count != 0) && (state == `S_FT601_TX_ACTIVE);
    assign can_enqueue  = din_wr_en && (FWD || (data_queue_count < 5));

    wire [63:0] ila_ft601_probe0 = {
        40'h0,
        FT601_WR_N,
        FT601_RD_N,
        FT601_OE_N,
    ...
    wire [31:0] ila_ft601_probe1 = din;
    wire [31:0] ila_ft601_probe2 = dout;
    wire [31:0] ila_ft601_probe3 = FT601_DATA_OUT[0];

    ila_ft601 i_ila_ft601 (
        .clk    ( clk               ),
        .probe0 ( ila_ft601_probe0  ),
        .probe1 ( ila_ft601_probe1  ),
        .probe2 ( ila_ft601_probe2  ),
        .probe3 ( ila_ft601_probe3  )
    );
    ...
    always @ ( posedge clk ) begin
        if ( rst || (data_cooldown_count == 4'hf) ) begin
            ...
            data_queue_count    <= 0;
            FT601_DATA_OUT[0]   <= 32'h00000000;
            FT601_DATA_OUT[1]   <= 32'h00000000;
            FT601_DATA_OUT[2]   <= 32'h00000000;
            FT601_DATA_OUT[3]   <= 32'h00000000;
            FT601_DATA_OUT[4]   <= 32'h00000000;
        end
        else begin
            data_cooldown_count <= (data_queue_count == 0) ? (data_cooldown_count + 1) : 0;
            data_queue_count    <= data_queue_count + (can_enqueue ? 3'b001 : 3'b000) - (FWD ? 3'b001 : 3'b000);
            if ( FWD ) begin
