...
module pcileech_pcie_tlp_a7(
    ...
    IfAXIS128 tlps_irq();       // RTL8168: MSI-X interrupt TLP stream

    // RTL8168: MSI-X vector table wires (from bar controller → interrupt generator)
    wire [31:0] w_msix_v0_addr_lo, w_msix_v0_addr_hi, w_msix_v0_data;
    wire        w_msix_v0_masked;
    wire [31:0] w_msix_v1_addr_lo, w_msix_v1_addr_hi, w_msix_v1_data;
    wire        w_msix_v1_masked;
    wire [15:0] w_bar2_isr;
    wire [1:0]  w_irq_req = 2'b00;  // tie to 0; drive from event logic when needed
    ...
    pcileech_tlps128_bar_controller i_pcileech_tlps128_bar_controller(
        ...
        // RTL8168: MSI-X table and ISR outputs
        .msix_v0_addr_lo ( w_msix_v0_addr_lo             ),
        .msix_v0_addr_hi ( w_msix_v0_addr_hi             ),
        .msix_v0_data    ( w_msix_v0_data                ),
        .msix_v0_masked  ( w_msix_v0_masked              ),
        .msix_v1_addr_lo ( w_msix_v1_addr_lo             ),
        .msix_v1_addr_hi ( w_msix_v1_addr_hi             ),
        .msix_v1_data    ( w_msix_v1_data                ),
        .msix_v1_masked  ( w_msix_v1_masked              ),
        .bar2_isr        ( w_bar2_isr                    )
    );

    // RTL8168: MSI-X interrupt generator
    rtl8168_interrupt_generator i_rtl8168_irq_gen(
        .rst             ( rst                           ),
        .clk             ( clk_pcie                      ),
        .pcie_id         ( pcie_id                       ),
        .v0_addr_lo      ( w_msix_v0_addr_lo             ),
        .v0_addr_hi      ( w_msix_v0_addr_hi             ),
        .v0_data         ( w_msix_v0_data                ),
        .v0_masked       ( w_msix_v0_masked              ),
        .v1_addr_lo      ( w_msix_v1_addr_lo             ),
        .v1_addr_hi      ( w_msix_v1_addr_hi             ),
        .v1_data         ( w_msix_v1_data                ),
        .v1_masked       ( w_msix_v1_masked              ),
        .irq_req         ( w_irq_req                     ),
        .irq_ack         (                               ),
        .tlps_irq_out    ( tlps_irq.source               )
    );
    ...
    // RTL8168: expanded 5-input mux (in5 = irq, lowest priority)
    pcileech_tlps128_sink_mux2 i_pcileech_tlps128_sink_mux2(
        .rst            ( rst                           ),
        .clk_pcie       ( clk_pcie                      ),
        .tlps_out       ( tlps_tx                       ),
        .tlps_in1       ( tlps_cfg_rsp.sink             ),   // priority 1: cfg shadow
        .tlps_in2       ( tlps_bar_rsp.sink             ),   // priority 2: BAR reads
        .tlps_in3       ( tlps_rx_fifo.sink             ),   // priority 3: DMA TX
        .tlps_in4       ( tlps_static                   ),   // priority 4: static TLPs
        .tlps_in5       ( tlps_irq.sink                 )    // priority 5: MSI-X irq
    );
...
// ------------------------------------------------------------------------
// RTL8168 PRIVATE FW: 5-input TLP TX mux
// Same logic as sink_mux1 but with an additional in5 (MSI-X interrupt, lowest priority).
// ------------------------------------------------------------------------
module pcileech_tlps128_sink_mux2 (
    input                       clk_pcie,
    input                       rst,
    IfAXIS128.source            tlps_out,
    ...
    wire [3:0] id_next          = ((id==0) || (tlps_out.tvalid && tlps_out.tlast))
                                   ? id_next_newsel : id;
